...
var context_menu  = {
    ...
    encrypt : function() {       
        ...
        //        window.openDialog("chrome://Cryption_Symmetric/content/options.xul", "", "chrome,titlebar,toolbar,centerscreen,modal", result);
        ...
        if (pass.value || prompts.promptPassword(window,"Password Dialog","Enter decryption password or passphrase",pass,"Remember password",remember))
            ...
            try {
                ...
                var encrypted = context_menu.encryptText(selection.toString(), pass.value);               

                var gClipboardHelper = Components.classes["@mozilla.org/widget/clipboardhelper;1"].getService(Components.interfaces.nsIClipboardHelper);
                gClipboardHelper.copyString(encrypted);

                if (!((target instanceof HTMLInputElement) || (target instanceof HTMLTextAreaElement))) {
                    window.openDialog("chrome://Cryption_Symmetric/content/options.xul", "", "chrome,titlebar,toolbar,centerscreen,modal", {
                        Password : pass.value ,
                        encryptSelection : selection,
